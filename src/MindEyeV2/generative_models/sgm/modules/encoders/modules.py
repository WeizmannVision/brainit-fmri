...
class GeneralConditioner(nn.Module):
    ...
    def forward(
        self, batch: Dict, force_zero_embeddings: Optional[List] = None, already_computed_clip_embeds = False
    ) -> Dict:
        # print(f"already_computed_clip_embeds: {already_computed_clip_embeds}")
        output = dict()
        if force_zero_embeddings is None:
            force_zero_embeddings = []
        for embedder in self.embedders:
            if isinstance(embedder, FrozenOpenCLIPImageEmbedder):# and already_computed_clip_embeds:
                emb_out = batch[embedder.input_key]
            else:
                ...
                with embedding_context():
                    if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                        ...
                        # print(f"embedder.input_key: {embedder.input_key}")
                        # print(f"batch[embedder.input_key]: {batch[embedder.input_key]}")
