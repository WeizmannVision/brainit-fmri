...
def unclip_recon_new(x, diffusion_engine, vector_suffix,
                 num_samples=1, offset_noise_level=0.04, init_image_latent=None,start_step=0):
    assert x.ndim==3
    if x.shape[0]==1:
        x = x[[0]]
    with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16), diffusion_engine.ema_scope():
        if init_image_latent is not None:
            z = init_image_latent.to(device)
            if z.shape[0] != num_samples:
                z = z.repeat(num_samples, 1, 1, 1)
        else:
            z = torch.randn(num_samples,4,32,32).to(device) # starting noise, can change to VAE outputs of initial image for img2img (96,96)
        ...
        sigma = sigmas[start_step].to(z.device)
        ...
        samples_z = diffusion_engine.sampler(denoiser, noised_z, cond=c, uc=uc,start_step=start_step) # amit start_step
        samples_x = diffusion_engine.decode_first_stage(samples_z)
        ### changed from original values to better fit the reconstructions 

        samples = torch.clamp((samples_x + 1.0) / 2.0, min=0.0, max=1.0)
        samples = (samples - 0.5) * 0.8 + 0.5 
